...
if($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($input_id_supermercato)) {
     ...
     if (empty($input_nome_reparto)) {
         $errors['nome_reparto'] = "Non hai inserito il codice del supermercato";
     } elseif (is_numeric($input_nome_reparto)) {
         $errors['nome_reparto'] = "Il codice deve essere composto da sole lettere";
     } else {
    ...
    // validazione di scaffale
    $input_scaffale =  (trim($_POST['scaffale']));
    if (empty($input_scaffale)) {
        $errors['scaffale'] = "Non hai inserito il codice dello scaffale del prodotto";
    } elseif (strlen($input_scaffale) > 4 ) {
        $errors['scaffale'] = "Il campo scaffale deve essere composto da massimo 4 caratteri";
    } else {
        $scaffale = $input_scaffale;
    }
    ...
    // controllo che la chiave codice del supermercato e nome del reparto sia presente nel database
    ...
    if (empty($row[0])) {
        $errors['id_supermercato'] = "Attenzione: la relazione nome del reparto - codice supermercato inserita non esiste";
    }

     // controllo se il codice del prodotto sia presente nel database
    ...
    if (strpos($_SERVER['REQUEST_URI'], 'nuovoProdotti_vendita') !== FALSE) {
        //controllo che la chiave non sia già presente nel DB
        $sql_search = "SELECT id_supermercato FROM prodotti_vendita WHERE id_supermercato = '$id_supermercato' AND nome_reparto = '$nome_reparto' AND id_prodotto = '$id_prodotto'";
        $result = pg_query($conn, $sql_search);     
        ...
        if (!empty($row[0])) {              //se la variabile non è vuota vuol dire che la chiave è già presente nel db 
            $errors['double'] = "Esiste già un prodotto su questo scaffale in quel supermercato";
        } 
    }
    //inserimento nel db
