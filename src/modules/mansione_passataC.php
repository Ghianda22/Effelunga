...
if($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    // validazione di descrizione
    $input_descrizione =  (trim($_POST['descrizione']));
    if (empty($input_descrizione)) {
        $errors['descrizione'] = "Non hai inserito la descrizione della mansione";
    } else {
    ...
    if(empty($input_data_fine)){
        $errors['data_fine'] = "Attenzione: non hai inserito la data fine";
    }else{
        $data_fine = $input_data_fine;
    }

    //validazione responsabile
    $input_responsabile = strtolower(trim($_POST["responsabile"]));
    if(empty($input_responsabile)){
        $errors['responsabile'] = "Attenzione: non hai spacificato se era responsabile o no";
    } elseif($input_responsabile != 'si' && $input_responsabile != 'no'){
        $errors['responsabile'] = "Attenzione: scrivere solo si o no";
    } else{
        if ($input_responsabile == 'si') {
            $responsabile = 1;
        } else {
            $responsabile = 0;
        }
    ...
    // controllo se il codice del supermercato/reparto è presente nel database
    ...
    if (strpos($_SERVER['REQUEST_URI'], 'nuovoMansione_passata') !== FALSE) {
        //controllo che la chiave non sia già presente nel DB
        $sql_search = "SELECT id_supermercato FROM mansione_passata WHERE id_supermercato = '$id_supermercato' AND nome_reparto = '$nome_reparto' AND cf = '$cf'";
        $result = pg_query($conn, $sql_search);     
        ...
        if (!empty($row[0])) {              //se la variabile non è vuota vuol dire che la chiave è già presente nel db 
            $errors['double'] = "Esiste già questa mansione passata per questo dipendente";
        } 
    }
    ...
    if(!array_filter($errors)){
        ...
        //se l'url che fa la richiesta contiene nuovoMansione_passata -> creare un Mansione_passata -> query insert
        if (strpos($_SERVER['REQUEST_URI'], 'nuovoMansione_passata') !== FALSE) {
            ...
            //se non c'è nuovoMansione_passata -> arrivo da modifyPHP
